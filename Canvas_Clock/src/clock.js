...
function init() {
    ...
    let canvas = document.getElementById("myCanvas"); //Создание холста объекта (let) холст из элемента HTML холст:
    let ctx = canvas.getContext("2d"); //Создание 2d объект чертежа (переменная CTX) для объекта холста:
    let radius = canvas.height / 2; //Вычислить радиус часов, используя высоту холста:

    ctx.translate(radius, radius); //Переназначить (0,0) положение (рисованного объекта) в центре холста:
    radius = radius * 0.9; //Уменьшить радиус часов (до 90%), чтобы сделать часы хорошо внутри холста:
